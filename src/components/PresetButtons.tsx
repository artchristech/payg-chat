...
export function PresetButtons({ onPresetClick }: PresetButtonsProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 max-h-96 overflow-y-auto hide-scrollbar">
      {[...Array(5)].map((_, index) => (
        <button
          key={index}
          onClick={() => onPresetClick(marketButton.prompt)}
          className="flex items-center gap-8 px-20 py-10 bg-gray-800 border border-gray-600 rounded-2xl hover:bg-gray-700 hover:border-gray-500 transition-all duration-200 text-3xl font-semibold text-gray-100 shadow-lg hover:shadow-xl hover:scale-105"
        >
